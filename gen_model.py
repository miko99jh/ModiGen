'''gen_model.py
This script generates text responses using a baseline or fine-tuned LLM for a set of prompts defined in a JSON file. 
It supports multi-run generation with retry logic to handle empty generations and saves outputs to structured files.
'''
import os
import time
import json
...
model_dir =  f"model_dir/Llama-3.1-8B-Instruct"  # Update with your model directory
model_name = "Llama-3.1-8B-Instruct"  # Update with your model name for output directory naming
...
context_file = f'context/0_Prompt_model.txt'
output_dir = f'result/{model_name}/output'
json_file_path = 'json_files/test_component_new.json'

# Read the JSON file
data = read_json(json_file_path)
instructions = [item['instruction'] for item in data[f'model']]

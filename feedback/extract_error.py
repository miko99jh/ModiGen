...
choice = f"GPT4o"
input_file = f'result/{choice}/all_check_results_by_model.json'
prompt_file = f'json_files/test_component.json'
model_directory = f'result/{choice}/model'  # Replace with the path to the folder containing .mo files
# Define the output directory

output_directory = f'result/feedback-{choice}/input(Error_Prompts)'  # Replace with your desired path
...
for model_key, model_list in data.items():
    for model in model_list:
        ...
        if model['error_msg'] and ("status" not in model):
            ...
            prompt_content = read_prompt_file(prompt_file, file_name_prefix, 'model')
